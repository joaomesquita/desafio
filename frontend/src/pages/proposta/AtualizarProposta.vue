<template>
  <v-flex class="pa-5" xs12>
    <v-card class="pa-10">
      <v-card-title>
        Atualizar Proposta da Licitação #{{ licitacao.licitacaoId }}
      </v-card-title>
      <v-card-text>
      ...
      <v-card-actions>
        ...
        <v-spacer></v-spacer>
        <v-btn class="primary" @click.prevent="atualizarProposta(proposta.propostaId)">
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.buscarProposta(this.$route.params.propostaId)
    },
    methods: {
      buscarProposta(propostaId) {
        try {
          PropostaService.buscarPorId(propostaId)
            .then(response => {
              this.proposta = response.data
              this.proposta.propostaId = propostaId
      ...
      atualizarProposta(propostaId) {
        try {
          PropostaService.atualizar(propostaId, this.proposta)
            .then(response => {
